...
function tree_sym_ISL_4(r, p_m, tau_1, tau_2, tau_3)
    ...
    time_ab_cd = tau_2 + rand(Exponential(1/(2*lam2)))
    time_abcd = tau_3 + rand(Exponential(1/lam4))
    if time_ab < tau_2
        flip = 1
        coal_time = time_ab
    elseif time_ab_cd < tau_3
        flip = dot([1 6],rand(Multinomial(1,[1/2,1/2])))
        coal_time = time_ab_cd
    else
...
function tree_sym_ISL_3(r, p_m, tau_2, tau_3, time_1, coal_1)
    lam2 = lam_2(r,p_m)
    lam3 = lam_3(r,p_m)
    if coal_1 == "(a,b)"
        time_cd = max(time_1, tau_2) + rand(Exponential(1/lam2))
    ...
    elseif coal_1 == "(c,d)"
        time_ab = time_1 + rand(Exponential(1/lam2))
        time_abcd = max(time_1, tau_3) + rand(Exponential(1/lam3))
        if time_ab < tau_3
            time = time_ab
            tree = "((a,b),(c,d))"
    ...
    elseif coal_1 == "(b,d)"
        ...
        time = max(time_1, tau_3) + rand(Exponential(1/lam3))
        flip = dot([1 2 3],rand(Multinomial(1,[1/3,1/3,1/3])))
        if flip == 1
            tree = "((a,c),(b,d))"
        elseif flip == 2
...
function ILS_main_sym(r, p_m, tau_1, tau_2, tau_3, nreps)
    coals = []
    for i=1:nreps
        first_coal = tree_sym_ISL_4(r, p_m, tau_1, tau_2, tau_3)
        time_1 = first_coal[1]
        coal_1 = first_coal[2]
        sec_coal = tree_sym_ISL_3(r, p_m, tau_2, tau_3, time_1, coal_1)
